...
if (isset($_GET['book_id'])) {
    ...
    $query = $pdo->prepare("SELECT * FROM on_going WHERE book_id = :book_id");
  ...
  <body>
    ...
    <main>
      <div class="__container__">
        ...
        <!-- Form Buku -->
        <section>
          <form id="formBuku" action="proseseditongoing.php" method="POST">
              <h2>Form Buku</h2>
              <input type="hidden" id="bukuId" name="book_id"  value="<?php echo $book['book_id']; ?>"/>
              ...
              <div class="formGrouping">
                <label>Halaman</label>
                <input type="number" class="inputText" id="inputHalaman" name="halaman"  value="<?php echo $book['halaman']; ?>"required />
              </div>
              <div class="mt_1 textRight">
                <button id="buttonDiscard" class="button buttonHapus" type="button" hidden>
                  Tutup
                </button>
                <button class="button buttonSimpan" type="submit" name="update">
                  Update
                </button>
              </div>
            </form>
